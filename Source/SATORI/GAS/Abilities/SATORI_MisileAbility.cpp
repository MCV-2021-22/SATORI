...
USATORI_MisileAbility::USATORI_MisileAbility() {
	...
	//Ability Tags
	FGameplayTag Tag = FGameplayTag::RequestGameplayTag(FName("Ability.Misile"));
	...
	const FGameplayEventData* TriggerEventData)
{
	...
	ASATORI_MisileActor* Misile = GetWorld()->SpawnActor<ASATORI_MisileActor>(
		MisileActor, //Actor to Spawn
		Character->GetActorLocation() + CameraForward * 100, //Spawn location
		Character->GetActorRotation()); //Spawn rotation

	FTimerHandle TimerHandle;
	FTimerDelegate TimerDelegate = FTimerDelegate::CreateUObject(this, &USATORI_MisileAbility::OnTimerExpiredFinish);
...
void USATORI_MisileAbility::OnTimerExpiredFinish()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, true);
}
