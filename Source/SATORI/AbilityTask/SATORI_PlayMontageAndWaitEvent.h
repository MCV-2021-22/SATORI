...
#include "SATORI_PlayMontageAndWaitEvent.generated.h"

class USATORI_AbilitySystemComponent;

/** Delegate type used, EventTag and Payload may be empty if it came from the montage callbacks */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FSATORIPlayMontageAndWaitForEvent, FGameplayTag, EventTag, FGameplayEventData, EventData);
 ...
 */
UCLASS()
class SATORI_API USATORI_PlayMontageAndWaitEvent : public UAbilityTask
{
	GENERATED_BODY()
	
public:
	// Constructor and overrides
	USATORI_PlayMontageAndWaitEvent(const FObjectInitializer& ObjectInitializer);
	...
	*/
	...
	FSATORIPlayMontageAndWaitForEvent OnCompleted;

	/** The montage started blending out */
	UPROPERTY(BlueprintAssignable)
	FSATORIPlayMontageAndWaitForEvent OnBlendOut;

	/** The montage was interrupted */
	UPROPERTY(BlueprintAssignable)
	FSATORIPlayMontageAndWaitForEvent OnInterrupted;

	/** The ability task was explicitly cancelled by another ability */
	UPROPERTY(BlueprintAssignable)
	FSATORIPlayMontageAndWaitForEvent OnCancelled;

	/** One of the triggering gameplay events happened */
	UPROPERTY(BlueprintAssignable)
	FSATORIPlayMontageAndWaitForEvent EventReceived;
	...
	UFUNCTION(BlueprintCallable, Category = "Ability|Tasks", meta = (HidePin = "OwningAbility", DefaultToSelf = "OwningAbility", BlueprintInternalUseOnly = "TRUE"))
		static USATORI_PlayMontageAndWaitEvent* PlayMontageAndWaitForEvent(
