...
void USATORI_ArqueroRotate::OnCancelled(FGameplayTag EventTag, FGameplayEventData EventData)
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, true);
}


void USATORI_ArqueroRotate::OnCompleted(FGameplayTag EventTag, FGameplayEventData EventData)
{
	//EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, true);
}


void USATORI_ArqueroRotate::EventReceived(FGameplayTag EventTag, FGameplayEventData EventData)
{
...
void USATORI_ArqueroRotate::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	Task->OnBlendOut.AddDynamic(this, &USATORI_ArqueroRotate::OnCompleted);
	Task->OnCompleted.AddDynamic(this, &USATORI_ArqueroRotate::OnCompleted);
	Task->OnInterrupted.AddDynamic(this, &USATORI_ArqueroRotate::OnCancelled);
	Task->OnCancelled.AddDynamic(this, &USATORI_ArqueroRotate::OnCancelled);
	Task->EventReceived.AddDynamic(this, &USATORI_ArqueroRotate::EventReceived);
