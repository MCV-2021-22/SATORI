...
#include "AI/Tasks/Fujin/BTTask_DesactivarTagInicial.h"
...
UBTTask_DesactivarTagInicial::UBTTask_DesactivarTagInicial()
{
	bCreateNodeInstance = true;
}

EBTNodeResult::Type UBTTask_DesactivarTagInicial::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if(Fujin)
	{
		Fujin->RemoveGameplayTag(FGameplayTag::RequestGameplayTag("Boss.Fujin.Esperando"));
		Fujin->RemoveGameplayTag(FGameplayTag::RequestGameplayTag("Boss.Fujin.Inicio"));
...
EBTNodeResult::Type UBTTask_DesactivarTagInicial::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	EBTNodeResult::Type Result = EBTNodeResult::Failed;
